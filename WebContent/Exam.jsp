...
<%@ page import="model.ExamDetailsModel"%>
...
<body>
	...
	<div class="col-sm-5 form-box ">
		...
		<div class="form-bottom">
			...
			<%
				DAO dao = new DAO();
				ArrayList<ExamDetailsModel> exam = dao.getExamDetails();

				if (!(session.getAttribute("type")).equals("1")) {
			%>

			<form role="form" action="PostExamDetails" method="post"
				...
				<div class="form-group">
					<label style="color: white" class="" for="role">Exam Date</label> <input
						type="date" name="examdate" placeholder="mm/dd/yyyy"
						class="form-last-name form-control" id="form-last-name" required>

				</div>

				<div class="form-group">
					<label style="color: white" class="" for="role">Details</label>
					<textarea name="details" class="form-control share-text" rows="3"
						placeholder="Exam Details" required></textarea>
			...
			<table style="color: white">
				<tr>
					...
					<th style="min-width: 10em">Exam Date</th>
				...
				%>
				<tr>
					<td><%=exam.get(i).getExamname()%></td>
					<td><%=exam.get(i).getExamdate()%></td>
					<td><%=exam.get(i).getDetails()%></td>
				</tr>
				<%
					}
				%>
		...
		<table style="color: white">
			<tr>
				...
				<th style="min-width: 10em">Exam Date</th>
				<th style="min-width: 10em">Details</th>
				<th style="min-width: 10em">Register</th>
			</tr>
			<%
				for (int i = 0; i < exam.size(); i++) {
			%>
			<tr>
				...
				<%
					ArrayList<String> registeredstudents = new ArrayList<>(
									Arrays.asList((exam.get(i).getRegistration())
											.split(",")));

				
							if (registeredstudents.contains((String) session
									.getAttribute("netid"))) {
				%>
				...
				%>
				<td><a href="RegisterExam?examid=<%=exam.get(i).getID()%>">Register</a></td>
